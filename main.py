...
async def open_profile_settings(page) -> None:
    ...
    for selector in menu_selectors:
        ...
        if await menu_button.count() > 0:
            try:
                ...
                menu_clicked = True
                print("Three-line menu clicked.")
                break
            except Exception:
                pass

    # Fallback: find an element containing a bars icon
    if not menu_clicked:
        bars_icon = page.locator(
            "i.fa-bars, "
            ".fa-bars, "
            "i.bi-list, "
            ".bi-list, "
        ...
        if await bars_icon.count() > 0:
            try:
                await bars_icon.click()
                await page.wait_for_timeout(700)
                menu_clicked = True
    ...
    if await my_account.count() > 0:
        try:
            ...
            await page.wait_for_timeout(700)
            print("My Account opened.")
        except Exception as error:
            print(
                "Could not click My Account: "
...
async def read_profile_field(
    ...
    """
    Reads an input value by looking for an exact label.
    ...
    # --------------------------------------------------------
    # Method 1: Search actual label elements
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Method 2: Search exact visible text and nearby input
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Method 3: Common input names and IDs
    # --------------------------------------------------------
...
async def login_and_scrape(
    ...
    # Open menu -> My Account -> Profile Settings
    # and fetch User ID and Your Name
    profile_information = (
